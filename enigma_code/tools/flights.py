...
def fetch_user_flight_information() -> list[dict]:
    ...
    """
    # print(f"CALLING THE '{fetch_user_flight_information.__name__}' FUNCTION")
...
def search_flights(
    ...
    print(f"CALLING THE '{search_flights.__name__}' FUNCTION")
    # TODO: Set the local file as the database to be used in the tutorial
    db = "travel2.sqlite"
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
...
def update_ticket_to_new_flight(ticket_no: str, new_flight_id: int) -> str:
    ...
    print(f"CALLING THE '{update_ticket_to_new_flight.__name__}' FUNCTION")
...
def cancel_ticket(ticket_no: str) -> str:
    ...
    print(f"CALLING THE '{cancel_ticket.__name__}' FUNCTION")
    config = ensure_config()
    configuration = config.get("configurable", {})
    passenger_id = configuration.get("passenger_id", None)
    if not passenger_id:
